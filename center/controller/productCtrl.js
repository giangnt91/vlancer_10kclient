coupon
	.controller('ProductCtrl', function ($scope, $location, $filter, $routeParams, $timeout, ngDialog, $window) {
		...
		if ($routeParams.danhmuc === 'ma-giam-gia-pho-thong') {
			...
			$scope.basic = $filter('orderBy')(allBasic, ['_id'], true);
		...
		if (shopId) {
			...
			DataServices.getByshopid(shopId).then(function (response) {
				if (response.data.error_code === 0) {
					...
					if ($scope.shop.shop_coupon.length > 0 && $scope.shop.shop_coupon[0].approved === true) {
					...
					if ($scope.shop.server_coupon.length > 0 && $scope.shop.server_coupon[0].approved === true) {
						$scope.o_coupon = "";
					...
					if ($scope.auth[0].total_list_coupon.length > 0) {
						for (var i = 0; i < $scope.auth[0].total_list_coupon.length; i++) {
							...
							if ($scope.shop.shop_coupon.length > 0) {
								...
								if ($scope.shop.shop_coupon[0].coupon.contains1($scope.auth[0].total_list_coupon[i].checkId.toString()) === true) {
									$scope._get = true;
									$scope.condition = true;
								} else {
									if ($scope._get !== true) {
							...
							// if(i < $scope.auth[0].total_list_coupon.length){
							// if($scope.shop.user_get_coupon.contains1($scope.auth[0].total_list_coupon[i]._id.toString()) === true) {
							// $scope._get = true;
							// $scope.condition = true;
							// }else{
							// if ($scope._get !== true) {
							...
							// check server coupon
								...
								// if(i < $scope.auth[0].total_list_coupon.length){
								if ($scope.shop.server_coupon[0].coupon.contains1($scope.auth[0].total_list_coupon[i].checkId.toString()) === true) {
									$scope._get = true;
									$scope.condition = true;
								} else {
									if ($scope._get !== true) {
					...
					if ($scope.auth[0].use_coupon.length > 0) {
						...
						for (var i = 0; i < $scope.auth[0].use_coupon.length; i++) {
							if ($scope.shop.shop_coupon.length > 0) {
								...
								if ($scope.shop.shop_coupon[0].coupon.contains1($scope.auth[0].use_coupon[i].checkId.toString()) === true) {
									$scope._get = true;
									$scope.condition = true;
								} else {
									if ($scope._get !== true) {
								...
								if ($scope.shop.server_coupon[0].coupon.contains1($scope.auth[0].use_coupon[i].checkId.toString()) === true) {
									$scope._get = true;
									$scope.condition = true;
								} else {
									if ($scope._get !== true) {
			...
			$scope.get_coupon = function () {
				...
				if ($scope.shop.shop_coupon !== null && $scope.shop.shop_coupon.length > 0) {
					first_coupon = {
						checkId: $scope.shop.shop_coupon[0].coupon[0].checkId,
					...
					if ($scope.shop.server_coupon !== null && $scope.shop.server_coupon.length > 0) {
						first_coupon = {
							checkId: $scope.shop.server_coupon[0].coupon[0].checkId,
				...
				DataServices.updateCoupon($scope.shop._id, the_issuer, JSON.stringify(new_list), JSON.stringify(first_coupon), $scope.auth[0]._id, new_slot).then(function (response) {
					if (response.data.error_code === 0) {
						...
						Thesocket.emit('user_get_coupon', $scope.auth[0].user_id, $scope.shop._id, $scope.shop.shop_info[0].shop_name);
						check_user_have_coupon();
						$("#af2").hide();
