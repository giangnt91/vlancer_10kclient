    ...
    .controller('ActionCtrl', function ($scope, $location, ngDialog, DataServices, $timeout, $filter, $window) {
       $scope.auth = JSON.parse(localStorage.getItem('auth'));
        ...
        if (!$scope.auth) {
            ...
            //     template:
            //         '<h4 class="flow-text green-text center">Đăng nhập để thực hiện tác vụ</h4> <br>' +
            ...
            DataServices.getActionperday($scope.auth[0].user_id).then(function (response) {
                if (response.data.list_action_per_day !== null) {
                            ...
                            if ($scope.list_action_per_day.length > 1) {
                                if ($scope.index === 0) {
                                    ...
                                    if ($scope.index < $scope.next_arr.length) {
                                                ...
                                                if (compare_reaction !== 0) {
                                                    DataServices.createReaction(JSON.stringify(reaction_kind), last_data.action_id, last_data.action_url, today, $scope.pre_data.action_shop_id, $scope.auth[0]._id).then(function (response) {
                                                        if (response.data.error_code === 0) {
                                                            DataServices.Point($scope.auth[0]._id, 10, 0).then(function (response) {
                                                                if (response.data.error_code === 0) {
                                                                    $scope.reaction_point = $scope.reaction_point + 10;
                                                                }
                                                            });
                                        ...
                                        //check comment after comment
                                        FB.api('/' + $scope.pre_data.action_shop_id + '_' + $scope.pre_data.action_id + '/comments?summary=true&order=reverse_chronological&access_token=' + accessToken, (response) => {
                                            var comment = response.summary;
                                            // if (comment.total_count) {
                                            $scope.pre_comment_2 = comment.total_count;
                                        ...
                                        function check_face() {
                                            ...
                                            FB.api('/' + last_data.action_shop_id + '_' + last_data.action_id + '/comments?summary=true&order=reverse_chronological&access_token=' + accessToken, (response) => {
                                                ...
                                                if (comment.total_count > $scope.pre_comment_2) {
                                                    DataServices.createReaction(JSON.stringify(reaction_kind), last_data.action_id, last_data.action_url, today, $scope.pre_data.action_shop_id, $scope.auth[0]._id).then(function (response) {
                                                        if (response.data.error_code === 0) {
                                                            $scope.array_com.push(last_data.action_url);
                                                        }
                                        ...
                                        FB.api('/' + $scope.pre_data.action_shop_id + '_' + $scope.pre_data.action_id + '/comments?summary=true&order=reverse_chronological&access_token=' + accessToken, (response) => {
                                            var comment = response.summary;
                                            // if (comment.total_count) {
                                            $scope.pre_comment_3 = comment.total_count;
                                            // console.log('binh luan 3 truoc khi comment: ' + comment.total_count, 'shop id: ' + $scope.pre_data.action_shop_id);
                                            // }
                                        });
                                    }
                                }
                        ...
                        $scope.complete = function () {
                            ...
                            function check_face() {
                                ...
                                FB.api('/' + last_data.action_shop_id + '_' + last_data.action_id + '/comments?summary=true&order=reverse_chronological&access_token=' + accessToken, (response) => {
                                    ...
                                    if (comment.total_count > $scope.pre_comment_3) {
                                        DataServices.createReaction(JSON.stringify(reaction_kind), last_data.action_id, last_data.action_url, today, $scope.pre_data.action_shop_id, $scope.auth[0]._id).then(function (response) {
                                            if (response.data.error_code === 0) {
                                                $scope.array_com.push(last_data.action_url);
                                            }
                    ...
                    // $scope.dialog = ngDialog.openConfirm({
                    //     template:
                    //         '<h4 class="flow-text green-text center">Bạn đã nhận đủ điểm cho hôm nay</h4> <br>' +
                    //         '<center><button type="button" class="waves-effect waves-light btn" style="" ng-click="confirm(confirmValue)">Ok</button></center>',
                    //     plain: true,
                    //     showClose: false,
                    //     closeByDocument: false
